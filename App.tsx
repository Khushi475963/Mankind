        ...
        <main className="flex-1 p-4 md:p-8 overflow-y-auto h-[calc(100vh-80px)]">
          ...
          {viewHistory.length > 0 && (
            ...
            >
              ...
              {selectedLanguage === 'Hindi' ? 'पीछे जाएँ' : selectedLanguage === 'Hinglish' ? 'Back Jayein' : 'Back to Previous'}
            </button>
          )}
          ...
          {currentView === ViewMode.DIGITAL_TWIN && (
            <div className="max-w-5xl mx-auto h-full animate-fade-in">
              <DigitalTwin records={records} currentPhone={currentUserPhone} selectedLanguage={selectedLanguage} />
            </div>
          )}

          {currentView === ViewMode.OUR_FACILITY && (
            <div className="max-w-6xl mx-auto h-full animate-fade-in">
              <OurFacility selectedLanguage={selectedLanguage} />
            </div>
          )}

          {currentView === ViewMode.DOCTORS_DIRECTORY && (
            <div className="max-w-6xl mx-auto h-full animate-fade-in">
              <DoctorsDirectory selectedLanguage={selectedLanguage} />
            </div>
          )}
